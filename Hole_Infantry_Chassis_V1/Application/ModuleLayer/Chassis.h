...
/* Minimal chassis application entry points and debugger status. */
typedef struct {
    uint8_t armed, ready, fault;
    float target_rpm[4];
    int16_t feedback_rpm[4], current_raw[4];
    uint32_t tx_errors;
} Chassis_Debug_t;
extern Chassis_Debug_t chassis_debug;
extern volatile uint32_t chassis_motor_rx_tick[4];
extern volatile uint8_t chassis_motor_seen;
