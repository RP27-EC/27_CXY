...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hcan, uint32_t RxFifo0ITs)
{
  ...
  if(hcan == &hfdcan1)
  {
		if (HAL_FDCAN_GetRxMessage(hcan, FDCAN_RX_FIFO0, &hcan1RxFrame.header, hcan1RxFrame.data) != HAL_OK ||
            hcan1RxFrame.header.IdType != FDCAN_STANDARD_ID ||
            hcan1RxFrame.header.RxFrameType != FDCAN_DATA_FRAME ||
            hcan1RxFrame.header.DataLength != FDCAN_DLC_BYTES_8) return;
...
HAL_StatusTypeDef CAN_SendData(FDCAN_HandleTypeDef *hcan, uint32_t stdId, uint8_t *dat)
{
	...
	status = HAL_FDCAN_AddMessageToTxFifoQ(hcan, &tx_message, dat);
