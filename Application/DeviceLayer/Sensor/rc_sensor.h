...
/* Exported macro ------------------------------------------------------------*/
//上板的开控设置为传输信息为开控并且板间通信正常

#define RC_ONLINE	 (Board_Rx_Info.state_pkt.car_state != 0 && Board_HeartBeat.status == DEV_ONLINE)

#define RC_OFFLINE (Board_Rx_Info.state_pkt.car_state == 0 || Board_HeartBeat.status == DEV_OFFLINE)
